    ...
    <div id="wrapper">
      ...
      <main v-show="connect">
        <connect-peer
          ...
          "
          @add-socket="(data) => socketMap.set(data.socketId, data.socket)"
        />
      </main>
    </div>
  </div>
  ...
  methods: {
    ...
    sendMessage() {
      ...
      if (this.peerData[this.peerId]) {
        ...
        if (
          this.message.startsWith("file:///") &&
          this.message.includes("?///")
        ) {
          const stream = ss.createStream();
          const part = this.message.substring(
            this.message.indexOf("file:///") + 8,
            this.message.indexOf("?///")
          );
          const filename = path.join(...part.split(/[/\\]+/g));
