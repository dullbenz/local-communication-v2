      ...
      <tab name="Listen">
        <div class="tabs-item">
          ...
          >
            ...
            <label
              v-if="listenSuccessLabel !== ''"
              class="connect-success-label"
            >{{ listenSuccessLabel }}</label>
            <label
              v-else-if="listenErrorLabel !== ''"
              class="connect-error-label"
            >{{ listenErrorLabel }}</label>
          </form>
        </div>
      </tab>
    </tabs>
...
export default {
  ...
  emits: ["add-socket", "new-connection"],
  data() {
    return {
      connecting: false,
      connectErrorLabel: "",
  ...
  watch: {
    connectedProps: {
      deep: true,
      handler() {
        if (this.connectedProps.peerId) {
          this.$emit("new-connection", this.connectedProps.peerId);
          this.connectedProps.peerId = "";
        }
      },
    },
  },
  mounted() {
    setVueFuncs({
      setConnectedProps: this.setConnectedProps,
    });
  },
  methods: {
    ...mapActions({
    ...
    handleListen() {
      if (!this.listening) {
        this.listenErrorLabel = "";
        if (this.peerData[this.listenName]) {
          return (this.listenErrorLabel = "Connection name already exists");
        }
    ...
    connectServer() {
      ...
      if (!this.connecting) {
        ...
        this.connectSuccessLabel = "";
        if (this.peerData[this.connectName]) {
          return (this.connectErrorLabel = "Connection name already exists");
        }
        this.connecting = true;
        socket = io(`${this.connectIp}:${config.socketPort}`, {
          transports: ["websocket"],
          reconnection: false,
        });
        socket.on("accept", () => {
          // this.connectErrorLabel = "";
          this.connectSuccessLabel = "Peer Connected";
          ...
          this.$emit("new-connection", this.connectName);
          ...
          ss(socket).on("streaming", async function (stream, data) {
            var filename = path.basename(data.name);

            try {
              const dirname = "./received";
            ...
            stream.pipe(fs.createWriteStream(path.join("received", filename)));
    ...
    setConnectedProps(props) {
      Object.keys(props).forEach((key) =>
        Vue.set(this.connectedProps, key, props[key])
      );
    },
  },
};
